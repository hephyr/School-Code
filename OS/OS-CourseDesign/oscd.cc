...
#define FILENAME "aa"
=======
>>>>>>> 22b0b764e51e13c0bb448b6dc48a2c0b04d5e56c
...
//DirItem用于存储文件与 目录信息，主要用途：将一个目录下的所有文件(包括目录)写入到该目录对应的Block中
typedef struct {
    unsigned short inum;
    char fileName[14];
}DirItem,*PtrDirItem;
...
<<<<<<< HEAD
bitset blockBitmap[BLOCKNUM];
bitset inodeBitmap[INODENUM];
=======
...
const unsigned short fcbSize=sizeof(DirItem);
...
void createFile(char *name,int flag)
{
    ...
    PtrDirItem fcb=(PtrDirItem)malloc(fcbSize);
    ...
    fseek(fp,behindDataBlock+
        INODENUM*inodeSize+parentInode->blockNum*BLOCKSIZE+parentInode->length*fcbSize,SEEK_SET);
    // 目录项写入父目录
...
void list()
{
    ...
    for(i=0;i<parentInode->length;i++)
    {
        ...
        // 判断目录还是文件
        fseek(fp, (behindDataBlock)+fcb->inum*inodeSize, SEEK_SET);
        ...
        fseek(fp, behindDataBlock+
            inodeSize*INODENUM+parentInode->blockNum*BLOCKSIZE+(i+1)*fcbSize, SEEK_SET);
    }
...
int findInodeNum(char *name,int flag)
{
    ...
    for(i=0;i<parentInode->length;i++)
    {
        ...
        if(flag==0)
        {
            if(strcmp(name,fcb->fileName)==0) // fcb->isDir == 0
            {
                fileInodeNum=fcb->inum;
                fseek(fp, behindDataBlock+fcb->inum*inodeSize, SEEK_SET);
                fread(fileInode, inodeSize, 1, fp);
                if (fileInode->isDir == 0)
                    break;
            }
        }
        else
        {
            if(strcmp(name,fcb->fileName)==0)
            {
                fileInodeNum=fcb->inum;
                fseek(fp, behindDataBlock+fcb->inum*inodeSize, SEEK_SET);
                fread(fileInode, inodeSize, 1, fp);
...
void open(char *name)
{
    ...
    else if ((fileInodeNum=findInodeNum(name,1))!=-1)
    {
...
void Delete(char *name)
{
    ...
    DirItem fcb[50]; // 可以使用new来动态保存fcb，不会有数量限制
