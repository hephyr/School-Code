...
void PCBCGSolver::dsprsax(double sa[], unsigned long ija[], double x[], double b[], unsigned long n){//内点的参数化坐标
...
void PCBCGSolver::dsprstx(double sa[], unsigned long ija[], double x[], double b[], unsigned long n)//同上
{
...
void PCBCGSolver::asolve(unsigned long n, double b[], double x[], int itrnsp)//给x[i]赋值，x[i]存的是坐标x，y的值，1~n是x，n~2n+1是y
{
	unsigned long i;
...
void PCBCGSolver::atimes(unsigned long n, double x[], double r[], int itrnsp)//优化内点，itrnsp为迭代次数
{
  if (itrnsp) dsprstx(sa,ija,x,r,n);//当inrnsp为非0值时
...
void PCBCGSolver::linbcg(unsigned long n, double b[], double x[], int itol, double tol,
	int itmax, int *iter, double *err)//用来解决一系列线性方程组，可以与论文中的解线性方程组相对应 ,进行内点的优化
{
		...
		if (itol == 1 || itol == 2) {
			...
			*err=snrm(n,r,itol)/bnrm;
			...
			if (fabs(zm1nrm-znrm) > EPS*znrm) {
				dxnrm=fabs(ak)*snrm(n,p,itol);//fabs取绝对值
