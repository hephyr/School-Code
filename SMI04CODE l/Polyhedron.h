...
class Polyhedron {
  public:
    ...
    */
    ...
    void readmesh(char *filename) { //读进信息
        ...
        for(i = 0; i < numberF; i++) {
            ...
            setFace(i, di, dj, dk);
            IDtool->AppendVFSort(i, FHead[Face[i][0]], FTail[Face[i][0]]); //将点加入面中
    ...
    void writemesh(char *filename) { //输出信息
        int i = 0;
    ...
    Polyhedron(const Polyhedron &rhs);//再次调用Polyhedron函数
  ...
  private:
    ...
    void BoundaryMap() { //定义边界
        ...
        for(i = 0; i < numberF; i++) {
            if(boundary[Face[i][0]] == 1 &&
                ...
                IDtool->AppendVF(Face[i][0], BedgeT); //将点添加进边
        ...
        while(next(now) != BedgeT) {
            now = next(now);
            tlength += PT->Distance(point[now->ID], point[next(now)->ID]);  //距离长度
    ...
    void memoryallocate(int dV, int dF) { //内存分配
    ...
    void setPoint(int i, double dx, double dy, double dz) { //设置点
    ...
    void SetBoundaryLines() { //设置边界线，与点有关
        int i = 0;
        numboundary = 0;
    ...
    void setFace(int i, int di, int dj, int dk) { //设置面
        Face[i] = new int[3];
        Face[i][0] = di;
        Face[i][1] = dj;
        Face[i][2] = dk;
        ...
        IDtool->AppendISort(dj, IHead[di], ITail[di], di, neighborI); //将ID添加进链表，并且数量加1
        IDtool->AppendISort(dk, IHead[di], ITail[di], di, neighborI);
        IDtool->AppendVF(dj, VTail[di]); //将myID添加进链表
    ...
    void setPolarMapNaturalB() { //极地图投影的自然边界
    ...
    void setPolarMap() { //极地图投影
        ...
        for(i = 0; i < numberV; i++) {
            if(boundary[i] != 1) {
                ...
                while(next(now) != VTail[i]) {
                    now = next(now);
                    PT->makeVector(bc[0], point[i], point[now->ID]); //法向量
                    PT->makeVector(bc[1], point[i], point[next(now)->ID]);
                    angle = acos((PT->InnerProduct(bc[0], bc[1]) / (PT->Point3dSize(bc[0]) * PT->Point3dSize(bc[1])))); //求出角度
                    theta += angle;
                    now = next(now);
                }
    ...
    void setAreaMap3D() { //设置3D面积图
        int i, j;
        IDList *now = NULL;
        sumarea3D = 0.0;
        for(i = 0; i < numberF; i++) {
            PT->makeVector(bc[0], point[Face[i][0]], point[Face[i][1]]); //法向量
            PT->makeVector(bc[1], point[Face[i][0]], point[Face[i][2]]);
            PT->CrossVector(bc[2], bc[1], bc[0]); //交叉矢量
    ...
    void ParametrizationSingle(int itenum, double error) { //Our single-step parameterization单步参数化
    ...
    void ParametrizationOptimal(int itenum, double error) { //最优参数化
        double *UaXY = new double[2 * (numberV) + 1];
        double *vecb = new double[2 * (numberV) + 1];
    ...
    // UOpt: Optimal low stretch parameterization (slow and smooth)//最优低伸缩参数化
    void ParametrizationSmoothOptimal(int itenum, double error) { //最优平面参数化
        double *UaXY = new double[2 * (numberV) + 1];
        double *vecb = new double[2 * (numberV) + 1];
        ...
        PCBCGSolver *mybcg = new PCBCGSolver(2 * nonzero); //与PCBCGSolver之间的关系
        double *sigsum = new double[numberV];
        if(weighttype == 0) {
            setFloaterC();//调用Floater
        } else if(weighttype == 1) {
            setLaplaceC();//调用拉普拉斯
        } else if(weighttype == 2) {
            setEckHC();
        ...
        // Re-solving linear system//解决线性方程
        double initialstrech = 0.0;
        double currentstrech = 0.0;
        Point2d **prevU = new Point2d* [numberV];
        for(i = 0; i < numberV; i++) {
    ...
    void setNaturalB(int itenum, double error) { //计算自然边界
        double *UaXY = new double[2 * (numberV) + 1];
        double *vecb = new double[2 * (numberV) + 1];
    ...
    void setEckHC() { //Eck et al. harmonic map  方法
        PolarList *nowp;
        int i;
        int nextID = 0;
        int backID = 0;
        ...
        for(i = 0; i < numberV; i++) {
            if(boundary[i] != 1) {
                ...
                while(next(nowp) != PTail[i]) {
                    ...
                    nowp->lambda = getHarmonicTerm(i, backID, nowp->ID, nextID); //调用getHarmonicTerm
    ...
    double getHarmonicTerm(int i, int backID, int nowID, int nextID) { //Eck et al. harmonic map 和谐  方法
        ...
        double area1 = 0.5 * PT->Point3dSize(bc[3]);
        if(area1 == 0.0)area1 = 1.0;
        PT->CrossVector(bc[3], bc[1], bc[2]);
        double area2 = 0.5 * PT->Point3dSize(bc[3]);
        ...
        ancos1 = (lenik1 * lenik1 + lenjk1 * lenjk1 - lenij) / area1; //角度
    ...
    void setDesbrunC() { //Desbrun et al. intrinsic parameterization
        ...
        for(i = 0; i < numberV; i++) {
            if(boundary[i] != 1) {
                ...
                while(next(nowp) != PTail[i]) {
                    ...
                    nowp->lambda = ((1.0 - intrinsiclambda) * getChiTerm(i, backID, nowp->ID, nextID) + intrinsiclambda * getAuthalicTerm(i, backID, nowp->ID, nextID)); //调用getChiTerm，getAuthalicTerm
                    ddweight += nowp->lambda;
                }
    ...
    double getAuthalicTerm(int i, int backID, int nowID, int nextID) { //Desbrun et al. intrinsic parameterization
        double ancos1, ancos2;
    ...
    double getChiTerm(int i, int backID, int nowID, int nextID) { //Desbrun et al. intrinsic parameterization
    ...
    void setMVCC() { //Sander et al. stretch minimizing parameterization??
        PolarList *nowp;
        int i;
        int nextID = 0;
        int backID = 0;
        ...
        for(i = 0; i < numberV; i++) {
            if(boundary[i] != 1) {
                ...
                while(next(nowp) != PTail[i]) {
                    ...
                    nowp->lambda = getMVCTerm(i, backID, nowp->ID, nextID); //调用getMVCTerm
                    ddweight += nowp->lambda;
                }
    ...
    double getMVCTerm(int i, int backID, int nowID, int nextID) { //Sander et al. stretch minimizing parameterization??
    ...
    void SortIndexPNaturalB() { //设置自然边界点的序列
        int i;
        IDList *now = NULL;
        PolarList *nowp = NULL;
    ...
    void setSigmaZero() { //局部弹性度，每个点的
        ...
        for(i = 0; i < numberF; i++) {
            ...
            dsize1 = PT->getParametricA(pV[Face[i][0]],//求面积
                                        pV[Face[i][1]],
                                        pV[Face[i][2]],
                                        pU[Face[i][0]],
                                        pU[Face[i][1]],
                                        pU[Face[i][2]]);
            PT->setParametricDs(bc[0], point[Face[i][0]], //特征值？？
            ...
            E[i] = PT->InnerProduct(bc[0], bc[0]); //内积
