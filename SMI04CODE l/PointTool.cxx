...
void PointTool::Normalize3D(Point3d* inout){//规范化3D,X,Y,Z
...
void PointTool::Normalize2D(Point2d* inout){//规范化2D,X,Y
...
double PointTool::getS(Point2d *v0,Point2d *v1,Point2d *v2){//读取2D中的3个点v0，v1，v2
  return (v1->x*v2->y-v1->y*v2->x - v0->x*(v2->y-v1->y) + v0->y*(v2->x-v1->x));//返回值
}
...
double PointTool::Point2DSize(Point2d *dv){//2D点的模
  return sqrt((dv->x*dv->x+dv->y*dv->y));

}
double PointTool::Point2DSize(double d1x,double d1y,double d2x,double d2y){//2点之间的距离
  double ddx = d1x-d2x;
  double ddy = d1y-d2y;
  return sqrt((ddx*ddx+ddy*ddy));

}
double PointTool::Point2DSizeSq(double d1x,double d1y,double d2x,double d2y){//2点之间距离的平方
  double ddx = d1x-d2x;
  double ddy = d1y-d2y;
  return (ddx*ddx+ddy*ddy);

}
void PointTool::ScalarVector(Point3d *out,double dv,Point3d *in){//标量向量
  out->x = dv*in->x;
  out->y = dv*in->y;
  out->z = dv*in->z;
...
void PointTool::setParametricDs(Point3d *out,Point3d *q1,Point3d *q2,Point3d *q3,double t1,double t2,double t3,double A){//求内点
  double dt1 = t2-t3;
  double dt2 = t3-t1;
  double dt3 = t1-t2;
...
double PointTool::getParametricA(double t1,double t2,double t3,double s1,double s2,double s3){//参数化面积
  double da = ((s2-s1)*(t3-t1)-(s3-s1)*(t2-t1))/2.0;//向量表示
  if(da==0.0)return 1.0;
  return da;
}  
double PointTool::Distance(Point3d *in1,Point3d *in2){//2点间距离
  return sqrt(((((in1->x)-(in2->x))*((in1->x)-(in2->x)))
	       +(((in1->y)-(in2->y))*((in1->y)-(in2->y)))
	       +(((in1->z)-(in2->z))*((in1->z)-(in2->z)))));
		      
  }
void PointTool::makeVector(Point3d *out,Point3d *in1,Point3d *in2){//矢量，法向量
    out->x = ((in2->x) - (in1->x));
    out->y = ((in2->y) - (in1->y));
    out->z = ((in2->z) - (in1->z));
}
void PointTool::CrossVector(Point3d *out,Point3d *in1,Point3d *in2){//交叉矢量
  out->x = ((in1->y)*(in2->z) - (in2->y)*(in1->z));
  out->y = ((in1->z)*(in2->x) - (in2->z)*(in1->x));
  out->z = ((in1->x)*(in2->y) - (in2->x)*(in1->y));
  }
double PointTool::InnerProduct(Point3d *in1,Point3d *in2){//c++封装的一个库函数，返回作为两个序列乘积而生成的元素的总和。步调一致地检查两个序列,将来自两个序列的元素相乘,将相乘的结果求和。由 init 指定和的初值。假定从beg2 开始的第二个序列具有至少与第一个序列一样多的元素,忽略第二个序列中超出第一个序列长度的任何元素。init 的类型决定返回类型
    return ((in1->x)*(in2->x) + (in1->y)*(in2->y) + (in1->z)*(in2->z));
  }
double PointTool::InnerProduct(Point2d *in1,Point2d *in2){//正交积
    return ((in1->x)*(in2->x) + (in1->y)*(in2->y));
  }
void PointTool::setCenter(Point3d *out,Point3d *in1,Point3d *in2,Point3d *in3){//设置中心
  out->x = (in1->x+in2->x+in3->x)/3.0;
  out->y = (in1->y+in2->y+in3->y)/3.0;
  out->z = (in1->z+in2->z+in3->z)/3.0;
  
}
double PointTool::Point3dSize(Point3d *in){//求模
    return sqrt(((in->x)*(in->x)+(in->y)*(in->y)+(in->z)*(in->z)));
  }


int PointTool::getBraycentricC(Point3d *evaluation,Point3d *baryparam,Point3d *v1,Point3d *v2,Point3d *v3,Point3d **bc){//质心的坐标
  double tA=0.0;
  double A1=0.0;
  double A2=0.0;
  double A3=0.0;
...
double PointTool::getArea(Point3d* dv1,Point3d* dv2,Point3d* dv3){//面
