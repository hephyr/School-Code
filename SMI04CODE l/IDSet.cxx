...
void IDSet::AppendVF(int myID,IDList *dVTail)//将myID添加进链表
    {
...
int IDSet::SearchI(int dID,IDList *dIHead,IDList *dITail){//查找
 ...
 void IDSet::AppendI(int dID,IDList *dIHead,IDList *dITail,int nowID,int *dnum)//附加
    {
      if(dID!=nowID){
...
void IDSet::AppendPolarI(int dID,PolarList *dITail,double dx,double dy){//附加极坐标？？
...
void IDSet::AppendVFSort(int dID,IDList *dIHead,IDList *dITail)//添加，将读进去的点添加到面中，有序
{
  IDList *now = new IDList(dID);
  IDList *dummy = dITail->back;
...
void IDSet::AppendISort(int dID,IDList *dIHead,IDList *dITail,int nowID,int *dnum){//将ID添加进链表，并且数量加1，有序
...
void IDSet::CleanNeighborPolar(PolarList* dHead,PolarList* dTail){//删除邻接点极坐标
...
void IDSet::CleanNeighbor(IDList* dHead,IDList* dTail){//删除邻接点
...
void IDSet::Clean(IDList **dFHead,IDList **dFTail,int numberSV,int *dneighborN){//清除
  IDList *now=NULL;
  IDList *dummy=NULL;
  int i;
